...
public class PlayGuess implements Playable, Serializable {
    ...
    public boolean callGame(Cell adventurer) {
        for (Cell toGuess : items) {
            if (adventurer.row == toGuess.row && adventurer.col == toGuess.col) {
                GameView.setGuessNumberCalled(true);
                return true;
            }
        }
        return false;
